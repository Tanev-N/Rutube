...
1. **Генерация кандидатов** (Candidate Generation)
   * Быстрый отбор потенциально релевантных видео.
2. **Ранжирование** (Ranking)
   * Точная сортировка по вероятности вовлечения.
3. **Фильтрация и бизнес-логика**
   * Удаление просмотренных, баланс новизны и релевантности.

---

#### **3. Рекомендации для новых пользователей (Cold Start Problem)**

У новых пользователей нет истории просмотров, поэтому используются стратегии:

**3.1. Неперсонализированные рекомендации**

* **Трендовые видео** :

```
  score = просмотры/время с загрузки + (лайки - дизлайки) - спам-фактор
```

* **Геолокация и язык** : показ популярного контента в регионе.

**3.2. Полуперсонализированные рекомендации**

* Профиль (возраст, пол).
* Если пользователь кликнул на одно видео → **content-based filtering** (похожие по тегам/описанию).
...
* User-based: "Похожим пользователям нравится X".
* Item-based: "Тем, кто смотрел A, нравится B".

1. **Двухбашенная модель** :

* Одна нейросеть кодирует пользователя, другая — видео.
* Похожесть определяется через скалярное произведение векторов.

1. **Графовые алгоритмы** :

* Видео и пользователи как узлы графа.

### **4.2. Ранжирование**

Кандидаты сортируются по:

* **Признакам пользователя** : история, демография.
* **Признакам видео** : retention rate, CTR.
* **Контексту** : время суток, устройство.

 **Модели** :

* GBDT (LightGBM, XGBoost) + Deep Learning (Transformer).
* Multi-Objective Optimization: оптимизация не только кликов, но и времени просмотра.

### **4.3. Пост-обработка**

* **Диверсификация** : избегание перекоса в одну тему.
* **Фрешность** : добавление новых видео

### **Алгоритмы оптимизации битрейта видео**

```
Битрейт = (размер видео в битах) / (длительность в секундах)
```

### **Типы битрейта**

1. **Constant Bitrate (CBR)** — фиксированный битрейт.
   * Плюсы: стабильная нагрузка на сеть.
   * Минусы: неэффективное использование bandwidth.
2. **Variable Bitrate (VBR)** — переменный битрейт.
   * Плюсы: лучше качество при том же размере.
   * Минусы: сложнее для стриминга.
3. **Average Bitrate (ABR)** — гибридный подход (используется в стриминге).


## **3. Оптимизация битрейта при кодировании**

### **3.1. Выбор битрейта на основе разрешения**

Примерные значения для H.264/AVC:

| Разрешение | Рекомендуемый битрейт (кбит/с) |
| -------------------- | ------------------------------------------------------- |
| 480p (SD)            | 1500–3000                                              |
| 720p (HD)            | 3000–6000                                              |
| 1080p (FHD)          | 6000–12000                                             |
| 4K (UHD)             | 20000–50000                                            |

 **Факторы влияния** :

* Движение в кадре (спорт vs. статичное видео).
* Цветовая насыщенность.
* Кодек (H.265/HEVC экономит 50% битрейта vs. H.264).

### **3.2. Психовизуальная оптимизация**

Кодеки используют особенности человеческого зрения:

* **Дискретное косинусное преобразование (DCT)** — сжатие текстур.
* **Квантование** — уменьшение детализации в менее заметных областях.
* **Motion Compensation** — предсказание движения для снижения избыточности.
...
Динамическая подстройка битрейта под сеть пользователя.

### **4.1. Как работает ABR?**

1. Видео кодируется в **несколько вариантов** (например: 500 кбит/с, 1500 кбит/с, 3000 кбит/с).
2. Клиент (браузер/приложение) выбирает подходящий вариант на основе:
   * Доступной bandwidth.
   * Размера буфера.
   * Загрузки CPU.

### **4.2. Популярные ABR-алгоритмы**

| Алгоритм           | Описание                                                                                                     |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **Buffer-Based**     | Выбор битрейта на основе заполненности буфера (например: BOLA, MPC). |
| **Hybrid**           | Комбинация bandwidth и буфера (YouTube использует такой подход).               |
| **Machine Learning** | Предсказание оптимального битрейта через нейросети (Netflix).          |
