...
| Graceful degradation              | Сервис для показа видео должен работать всегда.<br />При недоступности сервиса комментариев: срез топа на каждое видео + сохранение на них оценки + сохранение новых комментариев.<br />При недоступности сервиса рекомендаций показываюся горячие видео из сервиса HOT.<br />При недоступности сервиса поиска: Не показываем строку поиска<br />При недоступности сервиса аунтификации: Показываем интерфейс анонимного пользователя.<br />При недоступности сервиса пользователей: Не даем зайти пользователю в личный кабинет, измененением интерфейса.<br />При недоступности сервиса статистики: не показываем статистику. |
| Circuit breaker                   | failover policy - позволяет снять нагрузку с сервиса при недоступности одного микросервиса. Имеет 3 состояния: закрыт, когда всё работает; открыт, когда превышен лимит отказывов; полуоткрыт, когда происходит проверка работоспособности. Можно комбинировать с уменьшением запросов на проблемный хост, чтобы не прыгать между состояниями в случае, если много ошибок появляется именно под высокой нагрузкой                                                                                                                                                                                                                                                                                                                                                                                     |
| Логирование            | Использование трассировки для детального анализа цепочки запросов                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Мониторинг              | Отслеживание метрик                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Outbox                            | Ассинхронный паттерн для обработки изменений в сервисе статистики.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Go                   | легкая бизнес-логика   | 1000 | 100 Mb |
...
Для С3 только объем хранилища.

Добавить по ДЦ распределение

### Итоговая таблица

| Сервис                 | RPS (пик) | CPU (ядра) | RAM (Gb) | Сеть (ТБ/c) |
| ---------------------------- | ------------ | -------------- | -------- | ----------------- |
| Видео                   | 8 569       | 86             | 8.6      | 5                 |
| Комментарии       | 367          | 4              | 0.4      |                   |
| Рекомендации     | 2 452       | 245            | 24.5     |                   |
| Поиск                   | 1 800       | 180            | 18       |                   |
| Пользователи     | 10.14        | 1              | 0.1      |                   |
| Аутентификация | 4.14         | 1              | 0.1      |                   |
| Каналы                 | 4            | 1              | 0.1      |                   |
| Плейлисты           | 165          | 2              | 0.2      |                   |
| История               | 213          | 3              | 0.3      |                   |
| Статистика         | 8 000       | 8              | 0.8      |                   |

| БД                    | RPS (пик)                                                                                                                                      | CPU (ядра)                           | RAM (с индексами) | Disk (3 реплики)       |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | --------------------------- | ----------------------------- |
| **Cassandra**     | 8 569 (видео) + 367 (комменты) + 165 (плейлисты)  + каналы + юзеры + рекомендации =**12100** | 13 (x 4 для дц в репликах) | 1.1 TB                      | 4 TB * 3 =**12.5 TB**   |
| **ElasticSearch** | 1 800 (поиск)                                                                                                                               | 2                                        | 128 GB                      | 69.7 GB * 3 =**209 GB** |
| **Redis**         | 4.14 (Сесиии) + 8000 (Статистика) =8004.14                                                                                        | 9                                        | 30 GB                      | 50GB * 3 = 150 GB             |
| **ClickHouse**    | 213                                                                                                                                               | 2                                        | 60 GB                       | 51.5 GB * 3 =**155 GB** |

Облако S3 с запасом на 5 лет - 49Пбайт.

Через [какулятор](https://cloud.vk.com/pricing/) получается 108 904 руб/месяц.
На каждый ДЦ кол-во серверов расчитывается по таблице нагрузок.

Ставим примерно 1 балансер на 1k. Общее rps - 21684

Получается 22 балансера - 22 CPU.

Берем коэффициент запаса  1.6 (60%)

Общее кол-во CPU: 654

Общая RAM: 1560

(Посчитать аренду просто для серверов в VK CLOUD)

| Название железки                                                                                     | Хар-ки                                                                               | Цена в рублях | Кол-во, штук | Общая Цена в Мруб                      | Обслуживание своих машин в Мруб в год | Покупка в VK CLOUD в Мруб в год |
| ------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------------------ | --------------------- | ---------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------ |
| [Interprice сервер](https://granit-it.ru/configuration/granit/trinity-e100-m6-11/)                              | 8 гб опертивы / Intel Xeon E-2224 3.4GHz, 8M cache, 4C/4T + 10Gb/sec dual port | 171 264                  | 1047                  | 179.2 (20% для обсуживания в год) | 35.9                                                                 | 40.6                                             |
| [ssd](https://www.dns-shop.ru/product/97393ef7db76ed20/1600-gb-servernyj-ssd-micron-7450-max-mtfdkcb1t6tfc-1az1zabyy/) | 1600гб                                                                                  | 45 000                   | 11                    | 0.5                                                  | 0.1                                                                  | 1.3                                              |

| /                            | Москва (35%) | Санкт-Питербург (25%) | Астана (25%) | Владивосток (15%) |
| ---------------------------- | ------------------ | ----------------------------------- | ------------------ | ---------------------------- |
| Кол-во серверов | 367                | 262                                 | 262                | 158                          |

Вывод: Покупать свои железки выгоднее, чем арендовать у VK CLOUD.
