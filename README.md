...
| comments        | Cassandra     | `video_id`, `user_id`, `prev_comments_id`                                                                                                                 | Secondary Index       |
...
| playlist_videos | Cassandra     | `playlist_id`, `video_id`                                                                                                                                  | Secondary Index       |
...
* **comments** (579.3M записей) - шардировать по `video_id`
* **videos_rating** (1.79B записей) - шардировать по `video_id`
* **playlist_videos** (705M записей) - шардировать по `playlist_id`

### Репликация:

#### 1. **Таблицы с высокой частотой чтения (Read-Heavy)**

**Таблицы:** `videos`, `search_index`, `channels`, `users`
...
| ---------------------------------------------------------- | --------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Adaptive Bitrate Ladder (ABR)**                    | Транскодирование видео   | Автоматически генерирует версии видео (720p, 480p и др.) для адаптивного стриминга<br />под разные устройства и скорости интернета.                       |
| **Content-Aware Encoding (CAE)**                     | Оптимизация битрейта       | Умное сжатие видео: снижает битрейт для статичных сцен, сохраняя качество в динамичных.<br />Экономит трафик и хранилище.                                 |
| **Delta Encoding**                                   | CDN для видео                         | Передача только изменений между версиями видео (например, для 360p → 1080p)                                                                                                                           |
| **Two-Tower Neural Network**                         | Рекомендации (cold start)         | Предсказывает предпочтения новых пользователей на основе метаданных видео (теги, длительность)<br />и минимальных действий (1-2 просмотра). |
| **Weighted Multi-Armed Bandit**                      | Персонализация ленты       | Балансирует рекомендации между проверенными "хитами" и новым контентом. Учитывает CTR,<br />время просмотра и лайки.                                             |
| **AIMD (Additive Increase/Multiplicative Decrease)** | Битрейт-адаптация             | Динамически подстраивает качество под текущую пропускную способность сети.                                                                                                               |
